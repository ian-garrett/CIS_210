"""
   agenda.py: CIS 210 assignment 7, Winter 2014
...
class Agenda:
    ...
    def normalize(self): #Q somehow not detecting Emmanual's appointments
        """Sort and merge in agenda to eliminate overlaps

        args:
            self: the list of appointments
        returns:
            nothing
        Effects:
            modifies the self.appt by merging with overlaps
        """
        print ("Normalize reached!!!!")
        new_list = []
        sorted_list = sorted(self.appts, key=lambda appt: appt.begin.time())
        prev_begin = sorted_list[0].begin.time()
        prev_end = sorted_list[0].end.time()
        prev_desc = sorted_list[0].desc
        prev = sorted_list[0]
        for appt in sorted_list[1:]:
            begin = appt.begin.time()
            end = appt.end.time()
            desc = appt.desc
            if prev.overlaps(appt):
                prev_begin = min(appt.begin, prev.begin)
                prev_end = max(appt.end, prev.end)
                prev = (Appt(sorted_list[0].begin.date(),prev_begin.time(),prev_end.time(), prev_desc))
                #print ("Prev begin ==", prev_begin)
                #print ("Prev end ==", prev_end)
            else:
                # add (append) to new_list
                new_list.append(prev)
                prev = appt
                prev_end = end
                
        new_list.append(prev)
        #print ("NORMALIZE: new list ==", new_list)
        self.appts = new_list

    def complement(self, freeblock): #creates accurate compliment appts, but only does so for kieko
        ...
        for appt in self.appts:
            ...
            print ("begin time==",appt.begin.time())
            print ("appt==", appt)
            print ("freeblock", freeblock)
            # if appt is before freeblock, it doesn't need to be included
            if appt < freeblock:
                print("appt before freeblock")
                continue
            # if appt is after freeblock, it doesn't need to be included
            if appt > freeblock:
                print("appt after freeblock")
                continue
            # if appt is during freeblock, must add the times around it to the new agenda
            if current_time.time() < appt.begin.time():
                print("current time is before appt begin")
                new_agenda.append(Appt(date, current_time.time(), appt.begin.time(), freeblock.desc))
                current_time = appt.end
            # if current time is after beg of appt. and before appt.end
            if current_time.time() > appt.begin.time() and current_time.time() < appt.end.time():
                print("current_time reasigned to end of appointment")
                current_time = appt.end
        if current_time < freeblock.end:
            print ("currenttime less then freedblock")
