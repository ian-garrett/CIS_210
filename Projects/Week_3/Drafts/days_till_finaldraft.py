""".py: CIS 210 assignment 3, Winter 2014
...
def main():
    ...
    """ # is this function header ok? It is straight from skel code
    year = int(sys.argv[1])
    ...
    if not is_valid(year, start_month, start_day): # if components of start date given is not valid
        sys.exit("Must start on a valid date between 1800 and 2500")
    if not is_valid(2000, end_month, end_day): #if components of end date given is not valid
...
def is_valid(year, month, day): # how can I clean this up?
    """
    ...
    *day
    ...
    Effects: assigned one of two lists to active_months
    """
    
    if detect_leapyear(year):
        active_months = L_MONTHS
    else:
        active_months = MONTHS # make and statement
    if 1 <= month <= 12: # is the month within the acceptable range of 1-12?
        if 1 <= day <= active_months[month]: # is the day within 1 and the end month for a given month?
            if 1800 <= year <= 2500: # is the year within the range 1800 to 2500?
                return True
    else:
        return False
...
def days_between(year,start_month,start_day,end_month,end_day):
    ...
    year is not a leap year, then list MONTHS is assigned to active_months. After this assignment, the number of days of the months
    from start_month through end_month are 
