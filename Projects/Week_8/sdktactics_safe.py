...
def prepare(board):
    ...
    row_list = [ ]
    for row in range(9):
        row_list.append(board.tiles[row])
        groups.append(row_list)

    # Columns (we need new lists for these)
    col_list = [ ]
    for col in range(9):
        col_list.append(board.tiles[col])
        ...
        for group in groups:
            ...
            for tile in group:
                if tile not in seen:
                    ...
                    tile.announce('duplicate') #Ask about in office hours
...
def hidden_single(group):
        ...
        for sub_group in group:
            counter = 0
            shared_numbers = []
            for tile in sub_group:
                for number in need_placed:
                    if number in tile.possible:
                        shared_number.append(tile) #Q# why do we keep track of this?
                        counter += 1
                    if (len(tile.possible) == 1)and(tile.symbol == sdkboard.OPEN):
